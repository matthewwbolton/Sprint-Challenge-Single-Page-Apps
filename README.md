...
> Answer:
    ...
    Client-Side-Routing refers to the ability of the client (in this case the browser window) to handle routing, as opposed to a web server. The client is able to achieve this through its use of JavaScript. With client-side-routing, the client will make its initial request to the server and will in turn retrieve all the data that comprises the site in one large JavaScript file. As the user navigates the website, all of the routing will be handled internally in the window with JavaScript instead of having to make additional HTTP requests over a TCP/IP connection every time the user requests a new resource. This not only saves time and network resources, but it prevents the window from refreshing every single time and new request is made. Essentially, this means that the client is not ‘traveling to a new page as the user navigates the site, the window is merely changing what content is displayed.
